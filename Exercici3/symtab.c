...
void add_constant(const char *name, int type, int lineno)
{
    ...
    if (e == NULL) {
        ...
        /* if types match: silent reuse */
    }
}

/* ── get type of a simple identifier ───────────────────────────────────────── */

int get_id_type(const char *name, int lineno)
{
    Entry *e = lookup(name);
    if (e != NULL) return e->type;   /* found: variable or constant */
    ...
    /* define to suppress duplicate errors */
...
/* First occurrence WITH return type */
...
/* Subsequent occurrence WITHOUT return type */
