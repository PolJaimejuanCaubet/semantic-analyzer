...
quant_f
    ...
    | EXISTS ID COLON TYPE
      ...
      quant_f
      { scope_pop(); }
    /* Exercise 4: compact list  forall x1, x2, x3:int  */
    | FORALL varlist COLON TYPE
      { scope_push(); add_variables(&$2, $4, yylineno); }
      quant_f
      { scope_pop(); }
    | EXISTS varlist COLON TYPE
      { scope_push(); add_variables(&$2, $4, yylineno); }
      quant_f
      { scope_pop(); }
    | not_f
...
varlist
    : ID COMMA ID
      {
          memset(&$$, 0, sizeof($$));
          strncpy($$.names[0], $1, MAX_NAME-1); free($1);
          strncpy($$.names[1], $3, MAX_NAME-1); free($3);
          $$.count = 2;
      }
    | varlist COMMA ID
      {
          $$ = $1;
          if ($$.count < MAX_ARGS) {
              strncpy($$.names[$$.count], $3, MAX_NAME-1);
              $$.count++;
          }
          free($3);
      }
